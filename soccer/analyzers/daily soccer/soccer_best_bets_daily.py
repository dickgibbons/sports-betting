...
# OPTIMIZED CONFIGURATION (Based on full model retraining + backtest analysis)
# Post-retraining analysis (600 matches, 9 markets):
# ✅ BTTS: 100% training accuracy + $22K profit in backtest
# ✅ Over/Under 2.5: 100% training accuracy (perfect model!)
# 🚫 Match Outcome: 94% accuracy BUT lost $23K (calibration issue - DISABLED)
# ⚠️  Team Totals (H/A O/U 1.5): 88-91% accuracy but untested (DISABLED for now)
# 🚫 First Half: 74-77% accuracy (too low - DISABLED)
# 🚫 Over/Under 3.5: 75% accuracy (too low - DISABLED)
ENABLE_MATCH_WINNERS = False   # DISABLED - Lost $23K despite 94% accuracy (needs recalibration)
ENABLE_BTTS_YES = True        # ENABLE - 100% accuracy + proven profitable ($22K profit)
ENABLE_BTTS_NO = True         # ENABLE - 100% accuracy + proven profitable ($22K profit)
ENABLE_TOTALS = True          # ENABLE - 100% training accuracy (perfect model!)
ENABLE_TEAM_TOTALS = False    # DISABLED - Untested (enable after validation)
ENABLE_CORNERS = False        # DISABLED - No models trained
ENABLE_H1_TOTALS = True       # ENABLED - First half over/under 0.5 goals (per user request)

# LEAGUE-BASED FILTERING (Based on uncalibrated backtest - disabled for calibrated testing)
# With calibration, these leagues may perform differently - test first
LEAGUE_BLACKLIST = []  # Disabled for calibrated testing

# BTTS Yes whitelist - disabled for calibrated testing (calibration may help all leagues)
BTTS_YES_WHITELIST = []  # Disabled - test calibrated predictions across all leagues
...
class SoccerBestBetsGenerator:
    ...
    def __init__(self, model_path: str = None):
        ...
        if model_path is None:
            ...
            form_path = os.path.join(os.path.dirname(__file__), 'models', 'soccer_ml_models_with_form.pkl')
            enhanced_path = os.path.join(os.path.dirname(__file__), 'models', 'soccer_ml_models_enhanced.pkl')
            standard_path = os.path.join(os.path.dirname(__file__), 'models', 'soccer_ml_models.pkl')
    ...
    def load_calibration(self) -> dict:
        ...
        calibration_path = os.path.join(os.path.dirname(__file__), 'models', 'calibration_params.pkl')
    ...
    def fetch_upcoming_matches(self, date_str: str) -> List[Dict]:
        ...
        # Get top tier leagues only to limit volume
        top_leagues = self.leagues[self.leagues['tier'] <= 1]
        ...
        try:
            ...
            if match_probs is not None:
                ...
                if len(match_probs) == 3:
                    ...
                    # Home win
                    if ENABLE_MATCH_WINNERS and home_prob >= MIN_CONFIDENCE:
                    ...
                    # Draw
                    if ENABLE_MATCH_WINNERS and draw_prob >= MIN_CONFIDENCE:
                    ...
                    # Away win
                    if ENABLE_MATCH_WINNERS and away_prob >= MIN_CONFIDENCE:
            ...
            if btts_probs is not None:
                ...
                if len(btts_probs) == 2:
                    ...
                    if ENABLE_BTTS_YES and btts_yes_prob >= MIN_BTTS_CONFIDENCE:
                        ...
                        if not BTTS_YES_WHITELIST or league_name in BTTS_YES_WHITELIST:
            ...
            if ENABLE_H1_TOTALS and 'rf_first_half_over_0_5' in self.models and 'gb_first_half_over_0_5' in self.models:
                match_features = self.create_features_from_odds(odds)
                match_features_scaled = self.scaler.transform(match_features)
                rf_h1_05_probs = self.models['rf_first_half_over_0_5'].predict_proba(match_features_scaled)[0]
                gb_h1_05_probs = self.models['gb_first_half_over_0_5'].predict_proba(match_features_scaled)[0]
            ...
            if ENABLE_H1_TOTALS and 'rf_first_half_over_1_5' in self.models and 'gb_first_half_over_1_5' in self.models:
                match_features = self.create_features_from_odds(odds)
                match_features_scaled = self.scaler.transform(match_features)
                rf_h1_15_probs = self.models['rf_first_half_over_1_5'].predict_proba(match_features_scaled)[0]
                gb_h1_15_probs = self.models['gb_first_half_over_1_5'].predict_proba(match_features_scaled)[0]
