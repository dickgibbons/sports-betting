...
class RelaxedBacktester:
        ...
        try:
            ...
            if 'rf_match_outcome' in self.generator.models and 'gb_match_outcome' in self.generator.models:
                ...
                if len(match_probs) == 3:
                    ...
                    home_prob = self.generator.apply_calibration(home_prob, 'Home Win')
            ...
            if 'rf_over_2_5' in self.generator.models and 'gb_over_2_5' in self.generator.models:
                ...
                if len(ou_probs) == 2:
                    ...
                    # Apply calibration
                    under_prob = self.generator.apply_calibration(under_prob, 'Under 2.5')
            ...
            if 'rf_btts' in self.generator.models and 'gb_btts' in self.generator.models:
                ...
                if len(btts_probs) == 2:
                    ...
                    # Apply calibration
                    btts_no_prob = self.generator.apply_calibration(btts_no_prob, 'BTTS No')
            ...
            # Over/Under 1.5
            if 'rf_over_1_5' in self.generator.models and 'gb_over_1_5' in self.generator.models:
                rf_probs = self.generator.models['rf_over_1_5'].predict_proba(match_features_scaled)[0]
                gb_probs = self.generator.models['gb_over_1_5'].predict_proba(match_features_scaled)[0]
                ou_probs = (rf_probs + gb_probs) / 2

                if len(ou_probs) == 2 and 'over_15' in odds and 'under_15' in odds:
                    under_prob = ou_probs[0]
                    over_prob = ou_probs[1]

                    if over_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(over_prob, odds['over_15'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'Over 1.5',
                    ...
                    if under_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(under_prob, odds['under_15'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'Under 1.5',
            ...
            # Over/Under 3.5
            if 'rf_over_3_5' in self.generator.models and 'gb_over_3_5' in self.generator.models:
                rf_probs = self.generator.models['rf_over_3_5'].predict_proba(match_features_scaled)[0]
                gb_probs = self.generator.models['gb_over_3_5'].predict_proba(match_features_scaled)[0]
                ou_probs = (rf_probs + gb_probs) / 2

                if len(ou_probs) == 2 and 'over_35' in odds and 'under_35' in odds:
                    under_prob = ou_probs[0]
                    over_prob = ou_probs[1]

                    if over_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(over_prob, odds['over_35'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'Over 3.5',
                    ...
                    if under_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(under_prob, odds['under_35'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'Under 3.5',
            ...
            if 'rf_home_over_1_5' in self.generator.models and 'gb_home_over_1_5' in self.generator.models:
                rf_probs = self.generator.models['rf_home_over_1_5'].predict_proba(match_features_scaled)[0]
                gb_probs = self.generator.models['gb_home_over_1_5'].predict_proba(match_features_scaled)[0]
                team_probs = (rf_probs + gb_probs) / 2

                if len(team_probs) == 2 and 'home_over_15' in odds and 'home_under_15' in odds:
                    under_prob = team_probs[0]
                    over_prob = team_probs[1]

                    if over_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(over_prob, odds['home_over_15'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'Home Over 1.5',
                                'selection': f"{match.get('home_name', 'Home')} Over 1.5 Goals",
                                'odds': odds['home_over_15'],
                                'confidence': over_prob,
                                'kelly_stake': kelly,
                    ...
                    if under_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(under_prob, odds['home_under_15'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'Home Under 1.5',
                                'selection': f"{match.get('home_name', 'Home')} Under 1.5 Goals",
                                'odds': odds['home_under_15'],
                                'confidence': under_prob,
                                'kelly_stake': kelly,
                                'market_type': 'team_totals'
                            })

            # Away Team Over/Under 1.5
            if 'rf_away_over_1_5' in self.generator.models and 'gb_away_over_1_5' in self.generator.models:
                rf_probs = self.generator.models['rf_away_over_1_5'].predict_proba(match_features_scaled)[0]
                gb_probs = self.generator.models['gb_away_over_1_5'].predict_proba(match_features_scaled)[0]
                team_probs = (rf_probs + gb_probs) / 2

                if len(team_probs) == 2 and 'away_over_15' in odds and 'away_under_15' in odds:
                    under_prob = team_probs[0]
                    over_prob = team_probs[1]

                    if over_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(over_prob, odds['away_over_15'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'Away Over 1.5',
                                'selection': f"{match.get('away_name', 'Away')} Over 1.5 Goals",
                                'odds': odds['away_over_15'],
                                'confidence': over_prob,
                                'kelly_stake': kelly,
                                'market_type': 'team_totals'
                            })
                    ...
                    if under_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(under_prob, odds['away_under_15'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'Away Under 1.5',
                                'selection': f"{match.get('away_name', 'Away')} Under 1.5 Goals",
                                'odds': odds['away_under_15'],
                                'confidence': under_prob,
                                'kelly_stake': kelly,
                                'market_type': 'team_totals'
                            })
            ...
            if 'rf_h1_over_0_5' in self.generator.models and 'gb_h1_over_0_5' in self.generator.models:
                rf_probs = self.generator.models['rf_h1_over_0_5'].predict_proba(match_features_scaled)[0]
                gb_probs = self.generator.models['gb_h1_over_0_5'].predict_proba(match_features_scaled)[0]
                h1_probs = (rf_probs + gb_probs) / 2

                if len(h1_probs) == 2 and 'h1_over_05' in odds and 'h1_under_05' in odds:
                    under_prob = h1_probs[0]
                    over_prob = h1_probs[1]

                    if over_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(over_prob, odds['h1_over_05'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'H1 Over 0.5',
                                'selection': '1st Half Over 0.5 Goals',
                                'odds': odds['h1_over_05'],
                                'confidence': over_prob,
                                'kelly_stake': kelly,
                    ...
                    if under_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(under_prob, odds['h1_under_05'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'H1 Under 0.5',
                                'selection': '1st Half Under 0.5 Goals',
                                'odds': odds['h1_under_05'],
                                'confidence': under_prob,
                                'kelly_stake': kelly,
                                'market_type': 'h1_totals'
                            })
            ...
            if 'rf_h1_over_1_5' in self.generator.models and 'gb_h1_over_1_5' in self.generator.models:
                rf_probs = self.generator.models['rf_h1_over_1_5'].predict_proba(match_features_scaled)[0]
                gb_probs = self.generator.models['gb_h1_over_1_5'].predict_proba(match_features_scaled)[0]
                h1_probs = (rf_probs + gb_probs) / 2

                if len(h1_probs) == 2 and 'h1_over_15' in odds and 'h1_under_15' in odds:
                    under_prob = h1_probs[0]
                    over_prob = h1_probs[1]

                    if over_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(over_prob, odds['h1_over_15'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'H1 Over 1.5',
                                'selection': '1st Half Over 1.5 Goals',
                                'odds': odds['h1_over_15'],
                                'confidence': over_prob,
                                'kelly_stake': kelly,
                                'market_type': 'h1_totals'
                            })
                    ...
                    if under_prob >= MIN_TOTALS_CONFIDENCE:
                        kelly = self.generator.calculate_kelly(under_prob, odds['h1_under_15'])
                        if kelly > 0:
                            predictions.append({
                                'market': 'H1 Under 1.5',
                                'selection': '1st Half Under 1.5 Goals',
                                'odds': odds['h1_under_15'],
                                'confidence': under_prob,
                                'kelly_stake': kelly,
                                'market_type': 'h1_totals'
                            })
    ...
    def evaluate_prediction(self, prediction: Dict, actual_result: Dict) -> bool:
        ...
        # Over/Under 1.5
            ...
            return actual_result['total_goals'] > 1.5
        elif 'Under 1.5' in market:
            return actual_result['total_goals'] < 1.5

        # Over/Under 3.5
        elif 'Over 3.5' in market:
            return actual_result['total_goals'] > 3.5
        ...
        # Home Team Totals
        elif 'Home Over 1.5' in market:
            return actual_result['home_goals'] > 1.5
        elif 'Home Under 1.5' in market:
            return actual_result['home_goals'] < 1.5

        # Away Team Totals
        elif 'Away Over 1.5' in market:
            return actual_result['away_goals'] > 1.5
        elif 'Away Under 1.5' in market:
            return actual_result['away_goals'] < 1.5
