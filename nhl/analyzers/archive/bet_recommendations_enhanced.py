#!/usr/bin/env python3
"""
NHL Bet Recommendations Generator with Enhanced Models
Uses enhanced parameter models for better accuracy
Can fallback to standard models if enhanced not available
"""
...
from nhl_trainer_enhanced_params import NHLTrainerEnhancedParams


class BetRecommendationsEnhanced:
    """Generate bet recommendations using enhanced models"""
    ...
    def __init__(self, min_edge: float = 0.10, min_confidence: float = 0.62,
        ...
        self.trainer = NHLTrainerEnhancedParams()

    def load_models(self):
        """Load trained models - try enhanced first, fallback to standard"""

        # Try enhanced models first
        if self.use_enhanced:
            try:
                with open('nhl_models_enhanced_params.pkl', 'rb') as f:
                    data = pickle.load(f)
                ...
                print(f"✅ Loaded ENHANCED models ({len(self.models)} models)")
                print(f"   📈 300 estimators, deeper trees, improved accuracy")
                return
            except FileNotFoundError:
                print(f"⚠️  Enhanced models not found, falling back to standard models...")
            except Exception as e:
                print(f"⚠️  Error loading enhanced models: {e}")
                print(f"   Falling back to standard models...")
        ...
        try:
            ...
            print(f"✅ Loaded standard models ({len(self.models)} models)")
        except Exception as e:
    ...
    def get_todays_games(self, date_str: str = None):
        ...
        params = {'league': 57, 'season': 2025, 'date': date_str}
    ...
    def generate_recommendations(self, date_str: str = None) -> pd.DataFrame:
        ...
        for game in games:
            ...
            home_days_rest = self.trainer.get_days_since_last_game(home_team, game_time, 2025)
            away_days_rest = self.trainer.get_days_since_last_game(away_team, game_time, 2025)
    ...
    def _make_predictions(self, X_scaled, home_team, away_team, home_rest, away_rest):
        ...
        # Game Totals - Calculate from team predictions
        # Estimate total goals: combine team over probabilities
        # If both teams likely to score 2.5+, game total likely over 5.5
        # Simple heuristic: average of team totals gives game total probability
        home_expected_2_5 = preds['home_over_2_5_prob']
        away_expected_2_5 = preds['away_over_2_5_prob']

        # Game O/U 5.5 correlates with both teams scoring moderately
        # If both teams >60% to hit 2.5, game likely >5.5
        preds['game_over_5_5_prob'] = (home_expected_2_5 + away_expected_2_5) / 2

        # Game O/U 6.5 is more conservative - needs higher scoring
        home_expected_3_5 = preds['home_over_3_5_prob']
        away_expected_3_5 = preds['away_over_3_5_prob']
        preds['game_over_6_5_prob'] = (home_expected_3_5 + away_expected_3_5) / 2

        # P1 Team Totals - Estimate from full game probabilities
        # NHL P1 scoring: ~30% of total game goals happen in P1
        # Team that scores 2.5+ goals has ~65% chance to score 1+ in P1
        # Using O/U 0.5 (will team score in P1?)
        preds['home_p1_over_0_5_prob'] = home_expected_2_5 * 0.85  # Scale down
        preds['away_p1_over_0_5_prob'] = away_expected_2_5 * 0.85

        # P1 Game Total O/U 1.5
        # NHL average: 1.8 goals in P1
        # If both teams likely to score 2.5+ full game, P1 likely >1.5
        # Combine P1 team probabilities
        preds['p1_game_over_1_5_prob'] = (preds['home_p1_over_0_5_prob'] + preds['away_p1_over_0_5_prob']) / 2
    ...
    def _check_betting_opportunities(self, preds, home_team, away_team, date_str, game_time):
        ...
        # Check Game Total O/U 5.5
        g55_prob = preds['game_over_5_5_prob']
        g55_edge = self._calculate_edge(g55_prob, default_total_odds)

        if abs(g55_edge) >= self.min_edge:
            bet_confidence = g55_prob if g55_edge > 0 else (1 - g55_prob)
            if bet_confidence >= self.min_confidence:
                pick = 'Over' if g55_edge > 0 else 'Under'
                reason = f"Game Total {pick} 5.5 goals"
                ...
                bets.append({
                    ...
                    'Odds': default_total_odds,
                    'Confidence': f'{bet_confidence*100:.1f}%',
                    'Edge': f'{abs(g55_edge)*100:+.1f}%',
                    'Recommended_Unit': self._calculate_units(abs(g55_edge), bet_confidence),
                    'Reason': reason
                })

        # Check Game Total O/U 6.5
        g65_prob = preds['game_over_6_5_prob']
        g65_edge = self._calculate_edge(g65_prob, default_total_odds)

        if abs(g65_edge) >= self.min_edge:
            bet_confidence = g65_prob if g65_edge > 0 else (1 - g65_prob)
            if bet_confidence >= self.min_confidence:
                pick = 'Over' if g65_edge > 0 else 'Under'
                reason = f"Game Total {pick} 6.5 goals"
                ...
                bets.append({
                    ...
                    'Odds': default_total_odds,
                    'Confidence': f'{bet_confidence*100:.1f}%',
                    'Edge': f'{abs(g65_edge)*100:+.1f}%',
                    'Recommended_Unit': self._calculate_units(abs(g65_edge), bet_confidence),
                    'Reason': reason
                })

        # Check P1 Home Team Total O/U 0.5
        hp1_prob = preds['home_p1_over_0_5_prob']
        hp1_edge = self._calculate_edge(hp1_prob, default_total_odds)

        if abs(hp1_edge) >= self.min_edge:
            bet_confidence = hp1_prob if hp1_edge > 0 else (1 - hp1_prob)
            if bet_confidence >= self.min_confidence:
                pick = 'Over' if hp1_edge > 0 else 'Under'
                reason = f"{home_team} P1 {pick} 0.5 goals"

                bets.append({
                    'Game': game_str,
                    'Date': date_str,
                    'Time': game_time[:10] if len(game_time) > 10 else game_time,
                    'Bet_Type': f'{home_team} P1 {pick} 0.5',
                    'Pick': f'P1 {pick} 0.5',
                    'Odds': default_total_odds,
                    'Confidence': f'{bet_confidence*100:.1f}%',
                    'Edge': f'{abs(hp1_edge)*100:+.1f}%',
                    'Recommended_Unit': self._calculate_units(abs(hp1_edge), bet_confidence),
                    'Reason': reason
                })

        # Check P1 Away Team Total O/U 0.5
        ap1_prob = preds['away_p1_over_0_5_prob']
        ap1_edge = self._calculate_edge(ap1_prob, default_total_odds)

        if abs(ap1_edge) >= self.min_edge:
            bet_confidence = ap1_prob if ap1_edge > 0 else (1 - ap1_prob)
            if bet_confidence >= self.min_confidence:
                pick = 'Over' if ap1_edge > 0 else 'Under'
                reason = f"{away_team} P1 {pick} 0.5 goals"

                bets.append({
                    'Game': game_str,
                    'Date': date_str,
                    'Time': game_time[:10] if len(game_time) > 10 else game_time,
                    'Bet_Type': f'{away_team} P1 {pick} 0.5',
                    'Pick': f'P1 {pick} 0.5',
                    'Odds': default_total_odds,
                    'Confidence': f'{bet_confidence*100:.1f}%',
                    'Edge': f'{abs(ap1_edge)*100:+.1f}%',
                    'Recommended_Unit': self._calculate_units(abs(ap1_edge), bet_confidence),
                    'Reason': reason
                })

        # Check P1 Game Total O/U 1.5
        p1g_prob = preds['p1_game_over_1_5_prob']
        p1g_edge = self._calculate_edge(p1g_prob, default_total_odds)

        if abs(p1g_edge) >= self.min_edge:
            bet_confidence = p1g_prob if p1g_edge > 0 else (1 - p1g_prob)
            if bet_confidence >= self.min_confidence:
                pick = 'Over' if p1g_edge > 0 else 'Under'
                reason = f"P1 Game Total {pick} 1.5 goals"
                ...
                bets.append({
                    ...
                    'Odds': default_total_odds,
                    'Confidence': f'{bet_confidence*100:.1f}%',
                    'Edge': f'{abs(p1g_edge)*100:+.1f}%',
                    'Recommended_Unit': self._calculate_units(abs(p1g_edge), bet_confidence),
                    'Reason': reason
                })
    ...
    def save_recommendations(self, df: pd.DataFrame, date_str: str = None):
        ...
        model_suffix = "_enhanced" if self.model_type == 'enhanced' else ""
        filename = os.path.join(reports_dir, f"bet_recommendations{model_suffix}_{use_date}.csv")

        df.to_csv(filename, index=False)

        model_label = "ENHANCED" if self.model_type == 'enhanced' else "STANDARD"
        print(f"\n✅ Bet Recommendations Saved: {filename}")
        print(f"📊 {len(df)} recommended bets using {model_label} models")
        print(f"\n{'='*100}")
        print(f"🎯 TODAY'S BET RECOMMENDATIONS - {model_label} Models with Team Totals & B2B Analysis")
        print(f"{'='*100}\n")
...
def main():
    parser = argparse.ArgumentParser(description='Generate NHL bet recommendations with enhanced models')
    ...
    parser.add_argument('--standard', action='store_true',
                       help='Use standard models instead of enhanced')

    args = parser.parse_args()

    print("🏒 NHL BET RECOMMENDATIONS GENERATOR - Enhanced Models")
    print("=" * 100)
    print(f"Criteria: Min Edge {args.min_edge*100:.0f}% | Min Confidence {args.min_confidence*100:.0f}%")
    print("=" * 100)

    recommender = BetRecommendationsEnhanced(
