#!/usr/bin/env python3
"""
NHL Road Trip Backtest
Analyzes how teams perform on long road trips (3+, 4+, 5+ games)
...
class NHLRoadTripBacktest:
    """Backtest road trip performance patterns"""
    ...
    def __init__(self):
        ...
        print("🏒 NHL Road Trip Backtest initialized")

    def get_season_schedule(self, season: str):
        ...
        # Season runs Oct-Apr (roughly)
        start_date = datetime(season_year, 10, 1)
        end_date = datetime(season_year + 1, 4, 30)
        ...
        print(f"   ✅ Found {len(all_games)} completed games for {season} season")
        return all_games

    def build_road_trip_context(self, all_games: List[Dict]):
        """Build road trip context for each game"""

        print(f"\n🔍 Analyzing road trip patterns...")
        ...
        for game in all_games:
            ...
            # (Next game is home or no next game within 3 days)
    ...
    def display_results(self):
        ...
        print(f"\n{'='*80}")
        print(f"📊 ROAD TRIP BACKTEST RESULTS")
        print(f"{'='*80}\n")
    ...
    def run_backtest(self, seasons: List[str]):
        """Run backtest across multiple seasons"""

        print(f"\n{'='*80}")
        print(f"🏒 NHL ROAD TRIP BACKTEST")
        print(f"{'='*80}")
        ...
        for season in seasons:
            # Get season schedule
            all_games = self.get_season_schedule(season)
...
def main():
    parser = argparse.ArgumentParser(description='Backtest NHL road trip patterns')
    parser.add_argument('--seasons', nargs='+', default=['20222023', '20232024', '20242025'],
                       help='Seasons to analyze (format: 20222023)')

    args = parser.parse_args()

    backtester = NHLRoadTripBacktest()
    backtester.run_backtest(args.seasons)
