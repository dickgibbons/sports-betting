...
class DailyBettingReport:
    ...
    def __init__(self, track_bets: bool = True, include_soccer: bool = True):
    ...
    def _analyze_nhl_moneyline(self, home_team: str, away_team: str) -> List[Dict]:
        ...
        if b2b:
            ...
            edge = 7.6 if 'ROAD → ROAD' in b2b['reason'] else 6.7
            total_edge += edge
        ...
        if road_trip:
            ...
            total_edge += 6.5 if '4th' in str(road_trip.get('road_games', 0)) else 5.0
    ...
    def _analyze_nba_moneyline(self, home_team: str, away_team: str) -> List[Dict]:
        ...
        # Back-to-back (bigger edge in NBA!)
        b2b = self.nba_analyzer._analyze_back_to_back(home_team, away_team, self.date)
        if b2b:
            angles_found.append(b2b)
            edge = 12.0 if 'ROAD → ROAD' in b2b['reason'] else 8.0
            total_edge += edge
    ...
    def _collect_soccer_bets(self) -> List[Dict]:
        ...
        # Only collect HIGH confidence soccer bets (too many otherwise)
    ...
    def _display_top_bets(self, ranked_bets: List[Dict]):
        ...
        for bet in top_bets:
            ...
            # Icon
            sport_icon = {'NHL': '🏒', 'NBA': '🏀', 'SOCCER': '⚽'}.get(sport, '🎯')
    ...
    def save_report(self, ranked_bets: List[Dict]):
        ...
        with open(report_file, 'w') as f:
            ...
            for bet in ranked_bets[:10]:
                f.write(f"#{bet['rank']} {bet['sport']}: {bet['game']}\n")
